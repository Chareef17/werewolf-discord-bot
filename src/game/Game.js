...
class Game {
  ...
  async startGame() {
    if (this.players.size < 4) {
      return { success: false, msg: 'ต้องมีผู้เล่นอย่างน้อย 4 คน' };
    }
    ...
    await this.channel.send({
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(
            `ผู้เล่นทั้งหมด ${this.players.size} คนได้รับบทบาทแล้ว\nตรวจสอบ DM ของคุณเพื่อดูบทบาท!`
          )
          .addFields({
            name: '👥 ผู้เล่น',
    ...
    await this.channel.send({
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(
            `ความมืดปกคลุมหมู่บ้าน... ทุกคนหลับตา\nกลางคืนจะสิ้นสุด <t:${endTimestamp}:R>`
          )
  ...
  async sendDoctorDM(doctor) {
    ...
    if (this.lastDoctorTarget) {
      targets = targets.filter((p) => p.id !== this.lastDoctorTarget);
    }
    if (targets.length === 0) return;
  ...
  handleNightAction(userId, action, targetId) {
    if (this.phase !== 'night' || this.nightProcessed) {
      return { success: false, msg: 'ไม่ใช่เวลากลางคืน' };
    }
    ...
    switch (action) {
      case 'kill': {
        ...
        if (this.nightActions.werewolfVotes.has(userId))
          return { success: false, msg: 'คุณเลือกแล้ว' };
  ...
  async processNight() {
    ...
    if (voteCount.size > 0) {
      ...
      const topTargets = [...voteCount.entries()]
        .filter(([, v]) => v === maxVotes)
        .map(([id]) => id);
          ...
          await seer.user.send({
            embeds: [
              new EmbedBuilder()
                ...
                .setDescription(
                  `**${checked.displayName}** เป็น ${isWolf ? '🐺 **หมาป่า**!' : '👨‍🌾 **ฝ่ายชาวบ้าน**'}`
                ),
            ],
          });
        } catch {
          // Seer DM failed, continue
        }
      }
    }
    ...
    let resultText;
    if (!killTarget) {
      resultText = '🌅 ดวงอาทิตย์ขึ้นแล้ว... หมาป่าไม่ได้ลงมือคืนนี้\nไม่มีใครเสียชีวิต';
    } else if (saved) {
      resultText = '🌅 ดวงอาทิตย์ขึ้นแล้ว... หมอปกป้องเป้าหมายได้สำเร็จ!\nไม่มีใครเสียชีวิต';
    } else {
      resultText = `🌅 ดวงอาทิตย์ขึ้นแล้ว...\n💀 **${victim.displayName}** ถูกหมาป่ากัดตายเมื่อคืน!\nบทบาท: ${victim.role.emoji} ${victim.role.name}`;
    }
    ...
    if (victim && victim.role.id === 'hunter') {
      await this.channel.send({
        embeds: [
          new EmbedBuilder().setColor(COLOR.DEATH).setTitle('☀️ เช้าวันใหม่').setDescription(resultText),
        ],
      });
    ...
    await this.channel.send({
      embeds: [
        new EmbedBuilder().setColor(COLOR.DAY).setTitle('☀️ เช้าวันใหม่').setDescription(resultText),
      ],
    });
  ...
  async startDayDiscussion() {
    ...
    const livingList = this.getLivingPlayers()
      .map((p) => `• ${p.displayName}`)
      .join('\n');
    ...
    await this.channel.send({
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(
            `ถึงเวลาหารือกันว่าใครเป็นหมาป่า!\nการโหวตจะเริ่มขึ้น <t:${endTimestamp}:R>`
          )
          .addFields(
            { name: `🧍 ผู้เล่นที่ยังมีชีวิต (${this.getLivingPlayers().length})`, value: livingList }
          ),
      ],
    });
  ...
  async startDayVote() {
    ...
    const options = livingPlayers.map((p) => ({
      label: p.displayName,
      value: p.id,
      emoji: '🗳️',
    }));
    ...
    await this.channel.send({
      embeds: [
        new EmbedBuilder()
          ...
          .setTitle('🗳️ เวลาโหวต!')
          .setDescription(
            `เลือกผู้เล่นที่จะขับไล่ออกจากหมู่บ้าน\nการโหวตจะปิด <t:${endTimestamp}:R>`
          )
  ...
  handleDayVote(userId, targetId) {
    if (this.phase !== 'day_vote' || this.dayVoteProcessed) {
      return { success: false, msg: 'ไม่ใช่เวลาโหวต' };
    }

    const player = this.players.get(userId);
    if (!player || !player.alive) return { success: false, msg: 'คุณไม่สามารถโหวตได้' };

    if (targetId !== 'skip' && userId === targetId) {
      return { success: false, msg: 'ไม่สามารถโหวตตัวเองได้' };
    }
    ...
    if (targetId === 'skip') {
      return { success: true, msg: 'ข้ามโหวตแล้ว' };
    }
    const target = this.players.get(targetId);
  ...
  async processDayVote() {
    ...
    if (!eliminatedId) {
      ...
      await this.channel.send({
        embeds: [
          new EmbedBuilder().setColor(COLOR.DAY).setTitle('📊 ผลการโหวต').setDescription(resultText),
        ],
      });
    ...
    await this.channel.send({
      embeds: [
        new EmbedBuilder().setColor(COLOR.DEATH).setTitle('📊 ผลการโหวต').setDescription(resultText),
      ],
    });
    ...
    await this.channel.send({
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(
            `**${hunter.displayName}** เป็นนักล่า! สามารถเลือกยิงผู้เล่น 1 คนก่อนตาย`
          ),
      ],
    });
    ...
    try {
      ...
      await this.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(COLOR.DAY)
            .setDescription('🏹 นักล่าไม่ได้เลือกเป้าหมาย'),
        ],
      });
      this.hunterPending = false;
      return;
        ...
        if (this.hunterPending) {
          ...
          this.channel.send({
            embeds: [
              new EmbedBuilder()
                .setColor(COLOR.DAY)
                .setDescription('🏹 นักล่าไม่ได้เลือกเป้าหมายทันเวลา'),
            ],
          });
          resolve();
        }
    ...
    await this.channel.send({
      embeds: [
        new EmbedBuilder()
          ...
          .setTitle('🏹💥 นักล่ายิง!')
          .setDescription(
            `**${hunter.displayName}** ยิง **${target.displayName}** ก่อนตาย!\nบทบาท: ${target.role.emoji} ${target.role.name}`
          ),
      ],
    });
  ...
  async endGame(winnerTeam) {
    ...
    const title = isWolfWin
      ? '🐺 หมาป่าชนะ!'
      : '🏘️ ชาวบ้านชนะ!';
    const desc = isWolfWin
      ? 'หมาป่าครองหมู่บ้านแล้ว... ไม่มีใครรอดชีวิต'
      : 'ชาวบ้านกำจัดหมาป่าได้สำเร็จ! หมู่บ้านปลอดภัยอีกครั้ง';
  ...
  createLobbyEmbed() {
    ...
    return new EmbedBuilder()
      ...
      .addFields(
        {
          name: `👥 ผู้เล่น (${this.players.size}/16)`,
          value: playerList,
        },
        {
          name: '📌 ข้อมูลเกม',
          value: `ผู้เล่นขั้นต่ำ: 4 คน | สูงสุด: 16 คน\nเจ้าของห้อง: ${this.host.globalName || this.host.username}`,
        }
      )
  ...
  createLobbyComponents() {
    return new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId(`ww:join:${this.id}`)
        .setLabel('เข้าร่วม')
        .setStyle(ButtonStyle.Success)
        .setEmoji('✋'),
      new ButtonBuilder()
        .setCustomId(`ww:leave:${this.id}`)
        .setLabel('ออกจากเกม')
